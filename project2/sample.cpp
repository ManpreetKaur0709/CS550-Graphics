...
Display( )
{
	...
	if(whichview==OUTSIDE){
		...
		gluLookAt( 25.f, 10.f, 5.f,     0.f, 0.f, 0.f,     0.f, 1.f, -0.5f );
		...
		gluLookAt( 0.f, 1.f, 1.5f,     0.f, 5.f, 10.f,     0.f, 1.f, 0.f );
	...
	// first propler
	glPushMatrix();
	glTranslatef(0,1.5,7.8);
	glRotatef(angle_propeller,0,0,1);
	glScalef(5,1,1);
	glCallList( fan_List );
	glPopMatrix();
	...
	glPushMatrix();
	glTranslatef(-10,2,0);
	glRotatef(angle_propeller*2,0,1,-.3);
	glScalef(3,1,1);
	glRotatef(90,1,0,0);
	glCallList( fan_List );
	glPopMatrix();
...
{ 
	...
	fan_List = glGenLists( 1 );
	glNewList( fan_List, GL_COMPILE );
		glBegin( GL_TRIANGLES );
		    glColor3f(1.0,1.,1.);
