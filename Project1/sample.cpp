...
{ 
	...
	glNewList( BoxList, GL_COMPILE );
		...
		// UPPER SIDES CONNECTED WITH TRIANGLES

			glColor3f( 1., 0., 0. );

			glNormal3f( 1., 0., 0. );
					glVertex3f(  dx, -dy*0,  dz );
					glVertex3f(  dx, -dy*0, -dz );
					glVertex3f(  dx*0,  dy, -dz );
					glVertex3f(  dx*0,  dy,  dz );
      ...
      // SIDE QUADS
       ...
       // SIDES LEFT AND RIGHT
		...
		glBegin( GL_QUADS);
			...
			glNormal3f(0., 0., 1.);
					glVertex3f(.2*dx, -.5*dy, 0.001+dz);
					glVertex3f(.2*dx, -dy, .0001+dz);
					glVertex3f( -.2*dx,  -dy, .0001+dz);
					glVertex3f(-.2*dx,  -.5*dy, .0001+dz);
		...
		glBegin( GL_QUADS);
			...
			glNormal3f(0., 0., 1.);
					glVertex3f(.01*dx, -.5*dy, .0002+dz);
					glVertex3f(.01*dx, -dy, .0002+dz);
					glVertex3f( -.01*dx,  -.5*dy, .0002+dz);
					glVertex3f(-.01*dx,  -dy, .0002+dz);
	...
	glBegin( GL_QUADS);
       ...
       // positive x-axis side window

			glColor3f( 0., 0.5, 0. );

			glNormal3f( 1., 0., 0. );
					glVertex3f(  .01+dx, -0.1*dy,  .3*dz );
					glVertex3f(  .01+dx, -0.1*dy, -.3*dz );
					glVertex3f(  .01+dx,  -.5*dy, -.3*dz );
					glVertex3f(  .01+dx,  -.5*dy,  .3*dz );
        

		// negative x-axis side window
